...
async function fetchLiveQuotaCache() {
  ...
  for (const info of candidates) {
    ...
    for (const port of ports) {
      try {
        ...
        // 抓取 G1 credits (GOOGLE_ONE_AI)
        if (userStatus.userTier && Array.isArray(userStatus.userTier.availableCredits)) {
          const g1CreditObj = userStatus.userTier.availableCredits.find(c => c.creditType === 'GOOGLE_ONE_AI');
          if (g1CreditObj && g1CreditObj.creditAmount) {
            g1Credits = g1CreditObj.creditAmount;
          }
        }
  ...
  if (Object.keys(allModels).length > 0) {
    return { models: allModels, g1Credits, updatedAt: Date.now() };
  }
  return null;
}
